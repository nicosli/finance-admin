...
class ReportsController extends Controller
{
    public static function pastores($id_distrito, $mes, $anio){
        ...
        $informeMesAnioAnterior = Iglesias::with(['informes' => function($query) use ($mes, $anioAnterior){
            $query->where('mes_informe', $mes)->where('anio_informe', $anioAnterior);
        }, 'informes.remesa'])->where('id_distrito', '=', $id_distrito)->get();
        ...
        foreach ($informe as $key => $val) {
            foreach($val->informes as $key2 => $val2){
                ...
                foreach ($informeMesAnioAnterior as $key3 => $val3) {
